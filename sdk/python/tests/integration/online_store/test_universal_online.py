...
def test_write_to_online_store_event_check(environment):
    ...
    data = {
        ...
        "created_ts": [hour_ago, now, now],
    }
    dataframe_source = pd.DataFrame(data)
    data_source = environment.data_source_creator.create_data_source(
        df=dataframe_source,
        destination_name=f"test_write_to_online_store_time_check_{int(time.time_ns())}_{random.randint(1000, 9999)}",
    )
    ...
    data = {"id": [123], "string_col": ["hi_123"], "ts_1": [now], "created_ts": [now]}
    ...
    data = {
        ...
        "created_ts": [hour_ago, hour_ago, hour_ago],
    }
    df_data = pd.DataFrame(data)
...
def test_write_to_online_store(environment, universal_data_sources):
    if environment.test_repo_config.provider == "aws":
        # Checking timestamp during stream ingestion not done for AWS yet.
        pass
    fs = environment.feature_store
